            ...
            if file.endswith('.json'):
                ...
                try:
                    with open(file_path, 'r', encoding='utf-8-sig') as f:
                        for line in f:
                            if "iobSelection" in line:
                                json_associated_codes_count += 1
                                break  # Found the required key, no need to process further
                except Exception as e:
                    print(f"Skipping file {file_path} due to error: {e}")
            elif file.endswith('.csv'):
                csv_count += 1
                try:
                    chunk_iter = pd.read_csv(file_path, chunksize=1000, encoding='latin-1')
                    for chunk in chunk_iter:
                        if 'Associated_Codes' in chunk.columns:
                            csv_associated_codes_count += 1
                            break  # Found the required column, no need to process further
                except Exception as e:
